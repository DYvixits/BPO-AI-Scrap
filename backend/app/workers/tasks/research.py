...
async def _emit(job_id: uuid.UUID, kind: str, payload: dict[str, Any]) -> None:
...
async def _set_status(
    job_id: uuid.UUID, status: ResearchStatus, *, error: str | None = None
) -> None:
    async with database_module.async_session_factory() as db:
        await research_repository.set_status(db, job_id=job_id, status=status, error=error)
    await _emit(job_id, "status.changed", {"status": status.value})
...
async def run_research_job(ctx: dict[str, Any], job_id_str: str) -> None:
    ...
    try:
        ...
        # --- SEARCH ---
        await _set_status(job_id, ResearchStatus.SEARCHING)
        hits = await DuckDuckGoSearchProvider().search(job.query, max_results=max_results)
        await _emit(job_id, "search.completed", {"count": len(hits)})

        if not hits:
            await _set_status(job_id, ResearchStatus.COMPLETED)
            await _emit(job_id, "research.completed", {"result_count": 0})
            return
        ...
        async with database_module.async_session_factory() as db:
            for hit in hits:
                ...
                source = await research_repository.add_source(
                    db, job_id=job_id, url=hit.url, domain=domain
                )
                source_ids.append((source.id, hit.url))
        await _emit(job_id, "sources.discovered", {"count": len(source_ids)})

        # --- CRAWL (concurrent, SSRF-guarded) ---
        await _set_status(job_id, ResearchStatus.CRAWLING)
        fetcher = PageFetcher()
        ...
        # --- EXTRACT + STORE ---
        await _set_status(job_id, ResearchStatus.EXTRACTING)
        result_count = 0
        ...
        await _set_status(job_id, ResearchStatus.COMPLETED)
        await _emit(job_id, "research.completed", {"result_count": result_count})

    except Exception as exc:  # top-level job guard: never leave a job stuck mid-status
        logger.exception("research job %s failed", job_id)
        await _set_status(job_id, ResearchStatus.FAILED, error=str(exc))
        await _emit(job_id, "research.failed", {"error": str(exc)})
