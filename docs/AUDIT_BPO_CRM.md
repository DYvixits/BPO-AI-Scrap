...
| Intent Engine | PARTIAL — `app/engines/intent_scoring` aggregates a company's Commercial Signals into one score (mean decayed strength, unweighted by polarity); no per-signal-type weighting yet |
| Fit Scoring Engine | PARTIAL — `app/engines/fit_scoring` scores industry/geography/required_attributes match against the company's own evidence; company_size not scored (no headcount data extracted anywhere) |
| Opportunity Scoring Engine | PARTIAL — `app/engines/opportunity_scoring` combines Fit/Intent/Confidence/freshness/momentum via a fixed default weighting (`weights_used` recorded per row); not yet per-tenant-configurable (needs Configuration Engine, §56, still MISSING) |
...
| 8 | **Fit + Intent + Opportunity Scoring** (moved up from implicit position — this is the CRM-facing payoff) | Fit (objective-criteria match, nullable when nothing to check), Intent (mean decayed Commercial Signal strength), and Opportunity (fixed-weight combination of Fit/Intent/Confidence/freshness/momentum, per master spec §4's separate-tables architecture) ✅ done — see `docs/phases/PHASE_PLAN.md`'s Session 9 verification report; the weighting function is a single hardcoded default, not per-tenant-configurable (needs §56's Configuration Engine, still MISSING), and `momentum` is a same-snapshot proxy, not a real trend metric |
